...
What I created
- Per-app folder under `apps/<app>` with minimal scaffolds. For HelmRelease-based apps the files typically include: `kustomization.yaml`, `namespace.yaml`, `helm-release.yaml`, `pvc.yaml` (template), `secret-<app>.sops.yaml` (placeholders), and `httproute.yaml`.
- For custom-deployment apps (notably `aurral`, `musicseerr`, `soulsync`) the files typically include: `kustomization.yaml`, `namespace.yaml`, `deployment.yaml`, `service.yaml`, `secret-<app>.sops.yaml` (placeholders), and `httproute.yaml`.
...
- I did NOT populate chart names or repository references — I left only missing chart fields blank where I couldn't be 100% certain. Run the chart-discovery step and fill `chart.spec.chart` and `chart.spec.sourceRef.name` only where still missing in each `helm-release.yaml`. Do not run `helm template` against a Kustomize directory (e.g. `./apps/lidarr`); instead validate Flux resources. After you populate a HelmRelease's `chart` and `sourceRef`, use Flux CLI commands (for example `flux diff helmrelease <name> -n <namespace>` or `flux diff kustomization <name>`) to preview changes.
...
- create additional `*-media` PVCs and update HelmRelease values for other apps
- run a repo-wide scan and add `podSecurityContext` to every HelmRelease that references an NFS-backed PVC
- perform chart-discovery (I will only check upstream repos and report chart names/URLs; I will NOT add chart values without confirmation)
